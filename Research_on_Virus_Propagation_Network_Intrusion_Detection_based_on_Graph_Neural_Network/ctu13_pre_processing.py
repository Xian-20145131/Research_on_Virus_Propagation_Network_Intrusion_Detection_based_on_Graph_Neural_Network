...
def pre_processing(dataset_path: str):
    ...
    train.to_csv(os.path.join(dataset_path, 'CTU-13-train.csv'), index=False)
    val.to_csv(os.path.join(dataset_path, 'CTU-13-val.csv'), index=False)
    test.to_csv(os.path.join(dataset_path, 'CTU-13-test.csv'), index=False)

    # Create train, validation and test set for binary classification
    df['label'] = df['label'].apply(lambda x: 1 if x != 0 else 0)
    train, val = train_test_split(df, test_size=0.2, shuffle=True, random_state=42)
    train, test = train_test_split(train, test_size=0.25, shuffle=True, random_state=42)
    train.to_csv(os.path.join(dataset_path, 'CTU-13-train-binary.csv'), index=False)
    val.to_csv(os.path.join(dataset_path, 'CTU-13-val-binary.csv'), index=False)
    test.to_csv(os.path.join(dataset_path, 'CTU-13-test-binary.csv'), index=False)
