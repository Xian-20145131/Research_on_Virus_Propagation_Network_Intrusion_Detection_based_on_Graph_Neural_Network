...
def node_classification(dataset_path: str, binary, n_neigh, augmentation, hid, num_convs):
    ...
    _file_name_training = f'CTU-13-train{"-binary" if binary else ""}.csv'
    _file_path_training = os.path.join(dataset_path, 'raw', _file_name_training)
    _file_name_val = f'CTU-13-val{"-binary" if binary else ""}.csv'
    _file_name_testing = f'CTU-13-test{"-binary" if binary else ""}.csv'

    # Create training, val and test dataset
    train_dataset = CTU13NodeClassificationDataset(
        root=dataset_path,
    ...
    val_dataset = CTU13NodeClassificationDataset(
        root=dataset_path,
    ...
    test_dataset = CTU13NodeClassificationDataset(
        root=dataset_path,
